# -*- coding: utf-8 -*-
""""HW2_3_21_python_basics.ipynb"

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1zaNVuwfkE8NTano8ijVvYbsErVzhvMEB
...
def concatenate(seqs):
    return sum([list(seq) for seq in seqs], [])
...
def slices(seq):
    return (seq[start:end] for start in range(len(seq)) for end in range(start+1, len(seq)+1))
...
def digits_to_words(text):
    digits_names = ["zero", "one", "two", "three", "four",
                 "five", "six", "seven", "eight", "nine"]
    return ' '.join(digits_names[int(i)] for i in text if i.isdigit())
...
def calculated_work_time(df):
    ...
    for worker, group in df.groupby('WorkerId'):
        group = group.sort_values('AcceptTime').reset_index(drop=True)
        total_time = 0
        if group.empty:
            result[worker] = 0
            continue

        current_start = group.loc[0, 'AcceptTime']
        current_end = group.loc[0, 'SubmitTime']
        total_time += current_end - current_start

        for i in range(1, len(group)):
            s = group.loc[i, 'AcceptTime']
            e = group.loc[i, 'SubmitTime']
            ...
            if s > current_end:
                ...
                if e > current_end:
                    total_time += e - current_end
                    current_end = e

        result[worker] = total_time
...
def get_film_data(df_list):
    for df in df_list:
        if df.columns[0] == 'Rank' and 'Title' in df.columns:
            return df
...
def extract_revenue(df, column, name):
    df[name] = df[column].apply(lambda x: round(price_dec(x.split('[')[0]) / 1_000_000, 2))
    return df
